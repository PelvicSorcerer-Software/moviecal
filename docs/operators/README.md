...
6. **Queue / dispatch interaction** — state explicitly whether that platform may receive `Agent Dispatch = Yes` on a project item (see issue **#102** until the post-cutover policy is decided). Do not assert queue access on a platform's behalf.
