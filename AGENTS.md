...
- Optional operator PAT: add a GitHub PAT as `GITHUB_PAT_OPERATOR` (Runtime Secret) in Cursor Dashboard → Cloud Agents → Secrets. Project writes require a token with Projects API access; a repository-only fine-grained PAT is not enough.
  - **Recommended (classic PAT):** scopes `repo`, `read:org`, and `project`. `read:org` is required for `gh auth login` and `gh project` against user-owned projects.
  - **Fine-grained PAT (if used):** Repository (`PelvicSorcerer/moviecal`) → Issues and Pull requests (Read and write); Account → Projects (Read and write). If `gh auth login --with-token` reports `missing required scope 'read:org'`, switch to a classic PAT with the scopes above.
  - Do not name this secret `GH_TOKEN` or `GITHUB_TOKEN` — Cursor may inject its integration token as `GH_TOKEN`, and `gh` gives those variables precedence over stored credentials. The `.cursor/environment.json` `start` hook logs out and re-authenticates with `GITHUB_PAT_OPERATOR` when set.
